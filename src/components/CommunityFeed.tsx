...
import { ArrowUp, MessageCircle, MapPin, Search, Share2, Send, X } from 'lucide-react';
    ...
    <div className="relative min-h-screen pt-4 pb-24 page-enter">
      <div className={`relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 transition-all duration-300 ${showMapModal ? 'blur-md' : ''}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mt-6">
          ...
          {/* LEFT SIDEBAR */}
          <div className="hidden lg:flex flex-col gap-6 sticky top-20 self-start">
            <WeeklyDigest />
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* ── Instagram-style Stories (dynamic from reports) ── */}
            <div className="card p-4 border" style={{ borderColor: 'var(--border)' }}>
              <div className="flex gap-4 overflow-x-auto pb-1 scrollbar-none">
                {/* Add your story */}
                <div className="flex flex-col items-center gap-1.5 flex-shrink-0 cursor-pointer group">
                  <div className="w-14 h-14 rounded-full flex items-center justify-center border-2 border-dashed transition-all group-hover:scale-105"
                  ...
                  <span className="text-[10px] font-semibold text-center leading-tight w-14 truncate" style={{ color: 'var(--text-muted)' }}>Your Story</span>
                </div>

                {/* Dynamic stories from reports */}
                {(reports.length > 0 ? reports.slice(0, 8) : [
                  ...
                  return (
                    <div key={r.id || i}
                      onClick={() => handleSeenStory(r.id)}
                      className="flex flex-col items-center gap-1.5 flex-shrink-0 cursor-pointer group">
                      <div className="p-[2.5px] rounded-full transition-transform group-hover:scale-105"
                        style={{ background: isSeen ? 'var(--border-strong)' : 'linear-gradient(135deg, #f97316, #d94b38, #7c3aed)' }}>
                        <div className="p-[2px] rounded-full" style={{ background: 'var(--bg-base)' }}>
                          <div className="w-12 h-12 rounded-full flex items-center justify-center text-base font-extrabold"
                            style={{ background: bg, color: fg }}>
                            {initial}
                          </div>
                        </div>
                      </div>
                      <span className="text-[10px] font-semibold text-center leading-tight w-14 truncate"
            ...
            <div className="flex items-end justify-between mb-2">
              <div>
                <h1 className="text-3xl font-extrabold tracking-tight mb-1" style={{ color: 'var(--text-primary)' }}>Civic Feed</h1>
                <p className="text-sm font-medium" style={{ color: 'var(--text-muted)' }}>{reports.length} reports tracking the pulse of {location.city}</p>
              </div>
            </div>

            <div className="card p-4 shadow-sm border space-y-4" style={{ borderColor: 'var(--border)' }}>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: 'var(--text-faint)' }} />
                <input type="text" placeholder="Search by keyword, location, or department..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                  className="input-field pl-9 text-sm font-medium" />
              </div>
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
                {['All', 'Fire', 'Police', 'Medical', 'Infrastructure', 'Traffic'].map(dept => (
                  <button key={dept} onClick={() => setDepartmentFilter(dept === 'All' ? 'all' : `${dept} Department`)}
                    ...
                    style={{
                      background: (dept === 'All' && departmentFilter === 'all') || departmentFilter === `${dept} Department` ? 'var(--accent)' : 'var(--bg-elevated)',
          ...
          <div className="hidden lg:flex flex-col gap-6 sticky top-20 self-start">
            ...
            <div className="card h-[450px] overflow-hidden border shadow-sm" style={{ borderColor: 'var(--border)' }}>
              <Messages />
            </div>
