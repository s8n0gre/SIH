...
import { Clock, MapPin, AlertCircle, CheckCircle, MessageCircle, ThumbsUp, ThumbsDown, User } from 'lucide-react';
    ...
    try {
      ...
      if (response.ok) {
        ...
        const formattedIssues = reports.map((report: any) => ({
          ...
          status: report.status === 'resolved' || report.status === 'completed' ? 'resolved' :
            report.status === 'in-progress' || report.status === 'in_progress' || report.status === 'assigned' ? 'in-progress' :
              'pending',
      ...
      const mockIssues: Issue[] = [
        {
          ...
          location: `Main Street, ${userLocation.city}, ${userLocation.region}`,
