...
import ReportIssueV2 from './components/ReportIssueV2';
...
function App() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'feed': return <CommunityFeed searchTerm={searchTerm} departmentFilter={departmentFilter} setDepartmentFilter={setDepartmentFilter} />;
      case 'helpline': return <Helpline />;
      case 'admin': return <AdminDashboard />;
      default: return <CommunityFeed searchTerm={searchTerm} departmentFilter={departmentFilter} setDepartmentFilter={setDepartmentFilter} />;
    }
  };
  ...
  return (
    <ThemeProvider>
      <AIProvider>
        ...
        <div className={`${(showReportModal || isSearchOpen) ? 'blur-md pointer-events-none' : ''} transition-all duration-300`}>
          <Header 
            activeTab={activeTab} 
          ...
          {/* Report FAB */}
          <div className="fixed bottom-24 left-1/2 lg:left-[calc(33.333%-2rem)] -translate-x-1/2 z-50">
            <button
          ...
          <div className="mobile-scroll-container h-[calc(100vh-56px)] overflow-y-auto overflow-x-hidden">
        ...
        <AITaskIndicator onClick={() => setShowReportModal(true)} />
        ...
        {showReportModal && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center sm:p-4 backdrop-blur-sm">
            <div className="rounded-2xl sm:rounded-2xl rounded-t-2xl w-full sm:max-w-4xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto animate-fade-up" style={{ background: 'var(--bg-panel)', border: '1px solid var(--border)' }}>
              ...
              <div className="p-4"><ReportIssueV2 /></div>
            </div>
          </div>
        )}
      </div>
