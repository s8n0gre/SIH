...
const MapComponent: React.FC<MapComponentProps> = ({ 
  ...
  useEffect(() => {
    if (centerOnUserTrigger && leafletMapRef.current) {
      leafletMapRef.current.locate({ setView: true, maxZoom: 15, enableHighAccuracy: true });
    }
  }, [centerOnUserTrigger]);
  ...
  const createLeafletMap = (lat: number, lng: number) => {
    ...
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      attribution: hideControls ? '' : '© OpenStreetMap contributors'
    }).addTo(map);
